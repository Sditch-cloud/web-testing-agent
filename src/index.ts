...
const COMPILE_MAX_TOKENS = parseInt(
  process.env.COMPILE_MAX_TOKENS ?? "4096",
  10,
);
...
function checkTokenBudget(maxTokens: number): void {
  ...
  if (maxTokens > 8192) {
    console.warn(
      `[WARN] Compile max_tokens=${maxTokens} is unusually high — capped at 8192`,
    );
  }
}
...
export async function run(options: RunOptions): Promise<void> {
  ...
  if (resumeRunId) {
    ...
    if (saved.status === "completed") {
      console.log(
        `[INFO] Run ${resumeRunId} already completed — nothing to resume`,
      );
      return;
    }
    resumeState = saved;
    console.log(
      `[INFO] Resuming run ${resumeRunId} from step ${resumeState.step_cursor}`,
    );
  }
  ...
  if (!dsl) {
    if (!OPENAI_API_KEY) {
      throw new Error(
        "OPENAI_API_KEY is required. Copy .env.example to .env and fill in your key.",
      );
    }
    ...
    console.log(
      `[INFO] Compiled with confidence=${result.report.confidence.toFixed(2)}, tokens=${result.usage.total_tokens}`,
    );
